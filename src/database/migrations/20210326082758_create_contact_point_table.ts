...
export async function up(knex: Knex): Promise<void> {
  return knex.transaction(async (trx: Knex.Transaction) =>
    trx.schema
      ...
      .then(() =>
        trx.schema.hasTable(Table.contact_points).then((tableExists: boolean) => {
          if (!tableExists) {
            return trx.schema
              ...
              .createTable(Table.contact_points, (tableBuilder: Knex.CreateTableBuilder) => {
                tableBuilder
                  ...
                  .primary({ constraintName: `${Table.contact_points}_id` });
                tableBuilder.enum('use', ['home', 'work', 'temp', 'old', 'mobile']).notNullable();
                tableBuilder
                  .enum('system', ['phone', 'fax', 'email', 'pager', 'url', 'sms', 'other'])
                  .notNullable();
                tableBuilder.string('value');
                tableBuilder.enum('rank', [0, 1]);

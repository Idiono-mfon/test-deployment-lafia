...
export async function up(knex: Knex): Promise<void> {
  return knex.transaction(async (trx: Knex.Transaction) =>
    trx.schema
      ...
      .then(() =>
        trx.schema.hasTable(Table.patient_contacts_contact_points).then((tableExists: boolean) => {
          if (!tableExists) {
            return trx.schema
              .withSchema(Schema.lafiaService)
              .createTable(
                Table.patient_contacts_contact_points,
                (tableBuilder: Knex.CreateTableBuilder) => {
                  tableBuilder
                    ...
                    .primary({ constraintName: `${Table.patient_contacts_contact_points}_id` });
...
export async function down(knex: Knex): Promise<void> {
  return knex.schema
    .withSchema(Schema.lafiaService)
    .dropTableIfExists(Table.patient_contacts_contact_points);
}

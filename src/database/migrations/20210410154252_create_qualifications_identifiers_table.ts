...
export async function up(knex: Knex): Promise<void> {
  return knex.transaction(async (trx: Knex.Transaction) =>
    trx.schema
      ...
      .then(() =>
        trx.schema.hasTable(Table.qualifications_identifiers).then((tableExists: boolean) => {
          if (!tableExists) {
            return trx.schema
              .withSchema(Schema.lafiaService)
              .createTable(
                Table.qualifications_identifiers,
                (tableBuilder: Knex.CreateTableBuilder) => {
                  tableBuilder
                    ...
                    .primary({ constraintName: `${Table.qualifications_identifiers}_id` });
...
export async function down(knex: Knex): Promise<void> {
  return knex.schema
    .withSchema(Schema.lafiaService)
    .dropTableIfExists(Table.qualifications_identifiers);
}

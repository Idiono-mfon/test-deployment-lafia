...
export async function up(knex: Knex): Promise<void> {
  return knex.transaction(async (trx: Knex.Transaction) =>
    trx.schema
      ...
      .then(() =>
        trx.schema.hasTable(Table.components).then((tableExists: boolean) => {
          if (!tableExists) {
            return trx.schema
...
export async function down(knex: Knex): Promise<void> {
  return knex.schema.withSchema(Schema.lafiaService).dropTableIfExists(Table.components);
}
